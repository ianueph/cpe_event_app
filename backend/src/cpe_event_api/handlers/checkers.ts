...
export async function studentNumberExists(student_number : string) : Promise<boolean> {
    const result = await db.query({
        name: 'check-studentnumber-exists',
...
export async function studentIdExists(id : number) : Promise<boolean> {
    const result = await db.query({
        name: 'check-studentid-exists',
        text: 'SELECT 1 FROM students WHERE id = $1',
        values: [id]
    })
